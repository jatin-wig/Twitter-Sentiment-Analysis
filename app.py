...
tweet_input = st.text_area(" Type your tweet here:")
...
examples = [
    ...
    "Going to the gym later, need to stay consistent",
...
for example in examples:
    if st.button(example):
        ...
        if prediction == 1:
            st.markdown(f"<div class='result-card positive'>Sentiment: Positive </div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='result-card negative'>Sentiment: Negative </div>", unsafe_allow_html=True)
